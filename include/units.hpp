...
namespace sim {
    ...
    constexpr inline Q1 unit_cast(Q2 quantity) { return Q1(quantity.raw()); }
...
constexpr inline double to_##suffix(Name quantity) { return quantity.convert(ratio); }
    ...
    constexpr inline Q operator+(const Q& lhs, const Q& rhs) { return Q(lhs.raw() + rhs.raw()); }
    ...
    template<typename Q1, typename Q2>
    constexpr inline Division<Q1, Q2> operator/(const Q1& lhs, const Q2& rhs) {
    ...
    template<isQuantity Q>
    constexpr inline bool operator<(const Q& lhs, const Q& rhs) { return (lhs.raw() < rhs.raw()); }
    ...
    namespace units {
        template<isQuantity Q>
        constexpr inline Q abs(const Q& lhs) { return Q(std::abs(lhs.raw())); }

        template<isQuantity Q>
        constexpr inline Q max(const Q& lhs, const Q& rhs) { return (lhs > rhs ? lhs : rhs); }

        template<isQuantity Q>
        constexpr inline Q min(const Q& lhs, const Q& rhs) { return (lhs < rhs ? lhs : rhs); }
        ...
        pow(const Q& lhs) {
            return Quantity<std::ratio_multiply<decltype(Q::dim.mass), R>, std::ratio_multiply<decltype(Q::dim.dlenth),
                    R>,
                std::ratio_multiply<decltype(Q::dim.time), R>, std::ratio_multiply<decltype(Q::dim.angle), R>>(
        ...
        template<isQuantity Q>
        constexpr inline Q hypot(const Q& lhs, const Q& rhs) {
            return Q(std::hypot(lhs.raw(), rhs.raw()));
        }

        template<isQuantity Q>
        constexpr inline Q mod(const Q& lhs, const Q& rhs) {
            return Q(std::fmod(lhs.raw(), rhs.raw()));
        }

        template<isQuantity Q1, isQuantity Q2>
        constexpr inline Q1 copysign(const Q1& lhs, const Q2& rhs) {
            return Q1(std::copysign(lhs.raw(), rhs.raw()));
        }

        template<isQuantity Q>
        constexpr inline int sgn(const Q& lhs) { return lhs.raw() < 0 ? -1 : 1; }

        template<isQuantity Q>
        constexpr inline bool signbit(const Q& lhs) { return std::signbit(lhs.raw()); }

        template<isQuantity Q>
        constexpr inline Q clamp(const Q& lhs, const Q& lo, Q& hi) {
            return Q(std::clamp(lhs.raw(), lo.raw(), hi.raw()));
        }

        template<isQuantity Q>
        constexpr inline Q ceil(const Q& lhs, const Q& rhs) {
            return Q(std::ceil(lhs.raw() / rhs.raw()) * rhs.raw());
        }

        template<isQuantity Q>
        constexpr inline Q floor(const Q& lhs, const Q& rhs) {
            return Q(std::floor(lhs.raw() / rhs.raw()) * rhs.raw());
        }

        template<isQuantity Q>
        constexpr inline Q trunc(const Q& lhs, const Q& rhs) {
            return Q(std::trunc(lhs.raw() / rhs.raw()) * rhs.raw());
        }

        template<isQuantity Q>
        constexpr inline Q round(const Q& lhs, const Q& rhs) {
        ...
        template<isQuantity Q>
        constexpr inline isAngle asin(const Q& rhs) { return isAngle(std::asin(rhs.raw())); }

        template<isQuantity Q>
        constexpr inline isAngle acos(const Q& rhs) { return isAngle(std::acos(rhs.raw())); }

        template<isQuantity Q>
        constexpr inline isAngle atan(const Q& rhs) { return isAngle(std::atan(rhs.raw())); }
        ...
        template<isQuantity Q>
        constexpr inline isAngle asinh(const Q& rhs) { return isAngle(std::asinh(rhs.raw())); }

        template<isQuantity Q>
        constexpr inline isAngle acosh(const Q& rhs) { return isAngle(std::acosh(rhs.raw())); }

        template<isQuantity Q>
        constexpr inline isAngle atanh(const Q& rhs) { return isAngle(std::atanh(rhs.raw())); }

        template<isQuantity Q>
        constexpr inline isAngle atan2(const Q& lhs, const Q& rhs) {
    ...
    QUANTITY_NEW(Force, n, 1, 1, -2, 0)

    QUANTITY_NEW(Torque, nm, 1, 2, -2, 0)
